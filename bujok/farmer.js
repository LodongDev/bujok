...
const PHASE_CONFIG = {
    early: {
        name: '초반 (창병 파밍)',
        maxDistMinutes: 30,      // 편도 30분 이내
        mainUnit: 'spear',
        minPerTarget: { spear: 8 },
        maxPerTarget: { spear: 25, sword: 10 },
    },
    transition: {
        name: '전환기 (창병+경기병)',
        maxDistMinutes: 30,
        mainUnit: 'light',
        minPerTarget: { light: 3 },
        maxPerTarget: { spear: 15, sword: 5, light: 15 },
    },
    lc_dominant: {
        name: '중반 (경기병 위주)',
        maxDistMinutes: 40,
        mainUnit: 'light',
        minPerTarget: { light: 5 },
        maxPerTarget: { light: 40, spear: 5 },
    },
};
...
function calculateAllocation(available, farm, phase) {
    ...
    // 손실 있는데 약탈 낮음 → 스킵 (투자 대비 손해)
    if (losses > 0 && avgHaul < 50 && hasHistory) {
        return null; // null = 이 마을 건너뜀
    }
    ...
    if (phase === 'early') {
        ...
        const lc = available.light || 0;
        if (lc >= 5) {
            alloc.light = Math.min(10, lc);
            if (wall > 0) alloc.spear = 5;
        } else {
            alloc.spear = wall > 0 ? 20 : 10;
            alloc.light = lc > 0 ? Math.min(3, lc) : 0;
            if (wall > 0) alloc.sword = 5;
        }
    } else {
        if (wall === 0) {
            alloc.light = 8;
    ...
    // --- 히스토리 보정 ---
    if (hasHistory) {
        // 손실 있었고 약탈 높음 → 증파 (벽 극복 가치 있음)
        if (losses > 0 && avgHaul >= 50) {
            const boost = Math.min(losses, 3); // 최대 3단계 증파
            for (const unit of Object.keys(alloc)) {
                // 손실 1회당 주력 유닛 +30%
                alloc[unit] = Math.ceil(alloc[unit] * (1 + 0.3 * boost));
            }
        }

        // 약탈 풀(capacity 대비 100%) 기록이 있으면 소폭 증가
        const recentFull = (farm.history || []).slice(-3).filter(h => h.fullHaul).length;
        if (recentFull >= 2 && losses === 0) {
            // 최근 3회 중 2회 이상 풀 → 적재량 부족, 살짝 증파
            const mainUnit = phase === 'early' ? 'spear' : 'light';
            alloc[mainUnit] = Math.ceil((alloc[mainUnit] || 0) * 1.3);
        }
    }

    // max 캡 적용
    for (const [unit, max] of Object.entries(cfg.maxPerTarget)) {
        if (alloc[unit]) alloc[unit] = Math.min(alloc[unit], max);
    }

...
const PHASE_CONFIG = {
    early:       { name: '초반 (창병)', maxDistMin: 30, mainUnit: 'spear', minPerTarget: { spear: 8 }, maxPerTarget: { spear: 25, sword: 10 } },
    transition:  { name: '전환기 (혼용)', maxDistMin: 30, mainUnit: 'light', minPerTarget: { light: 3 }, maxPerTarget: { spear: 15, sword: 5, light: 15 } },
    lc_dominant: { name: '중반 (LC)', maxDistMin: 500, mainUnit: 'light', minPerTarget: { light: 3 }, maxPerTarget: { light: 50, ram: 10 } },
};
...
function calculateAllocation(pool, farm, phase) {
    ...
    if (phase === 'early') {
        if (wall === 0) alloc.spear = 10;
        else if (wall <= 2) { alloc.spear = 20; alloc.sword = 5; }
        else { alloc.spear = 25; alloc.sword = 10; }
    } else if (phase === 'transition') {
        const lc = pool.light || 0;
        if (lc >= 5) { alloc.light = Math.min(10, lc); if (wall > 0) alloc.spear = 5; }
        else { alloc.spear = wall > 0 ? 20 : 10; alloc.light = lc > 0 ? Math.min(3, lc) : 0; if (wall > 0) alloc.sword = 5; }
    } else {
        // LC 중반: 예상 자원에 맞춰 LC 수량 결정
        const estRes = estimateResources(farm);
        let base = Math.max(3, Math.ceil(estRes * 1.2 / UNIT_CARRY.light)); // 20% 버퍼

        // 벽 보정
        if (wall > 0) base = Math.ceil(base * 1.5);

        alloc.light = Math.min(base, pool.light || 0);
    }
    ...
    if (hasHistory) {
        if (losses > 0 && avgHaul >= 50) {
            ...
            for (const u of Object.keys(alloc)) alloc[u] = Math.ceil(alloc[u] * (1 + 0.3 * boost));
        }
        const recentFull = (farm.history || []).slice(-3).filter(h => h.fullHaul).length;
        if (recentFull >= 2 && losses === 0) {
            const main = phase === 'early' ? 'spear' : 'light';
            alloc[main] = Math.ceil((alloc[main] || 0) * 1.3);
        }
    }

    // 손실 있는 마을 → 램 추가 (벽 깎기, 벽 정보 없어도 보냄)
    // 램 속도 30분/칸이라 느려지지만, 벽 안 깎으면 계속 손실
    if (losses > 0 && (pool.ram || 0) > 0) {
        const ramCount = wall <= 2 ? 2 : Math.min(5, wall);
        alloc.ram = Math.min(ramCount, pool.ram || 0);
    }

    for (const [u, max] of Object.entries(cfg.maxPerTarget)) {
        if (alloc[u]) alloc[u] = Math.min(alloc[u], max);
    }
